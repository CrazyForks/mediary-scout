...
const { createAgentModelFromEnv, runInterrogation, buildTvAnimeSystemPrompt } = await import(
  path.join(repoRoot, "packages/workflow/dist/index.js")
);

const scenario = args.scenario ?? "Target: 莉可丽丝 (Lycoris Recoil) season 1, missing S01E01-S01E13.";
...
const transcript = await runInterrogation({
  ...
  systemPrompt: buildTvAnimeSystemPrompt(promptOptions),
  scenario,
});
